...
public
class CTArtifact implements CTSerializable
{
    ...
    public
    CTArtifact(CTVec3 location, CTLineType lineType, CTArtifactType artifactType, short ticks)
    {
    ...
    List<CTLine> makeLines()
    {
        ...
        if (this.artifactType.equals(CTArtifactType.BLOCKBOX))
        {
            ...
            {
                lines.add(new CTLine(this.location.plus(edge.start), this.location.plus(edge.finish), this.lineType,
                    this.ticks));
            }
        }
    ...
    boolean equals(Object other)
    {
        if (other instanceof CTArtifact)
        {
            ...
            return this.location.equals(otherArtifact.location) && this.lineType.equals(otherArtifact.lineType) &&
                   this.artifactType.equals(otherArtifact.artifactType) && this.ticks == otherArtifact.ticks;
        }
    ...
    int hashCode()
    {
        return new HashCodeBuilder(17, 31).append(this.location).append(this.lineType.ordinal())
            .append(this.artifactType.ordinal()).append(this.ticks).toHashCode();
    }
    ...
    public static
    CTArtifact fromBytes(DataInputStream dis, short ticks) throws IOException
    {
        return new CTArtifact(CTVec3.fromBytes(dis), CTLineType.fromBytes(dis), CTArtifactType.fromBytes(dis), ticks);
    }
