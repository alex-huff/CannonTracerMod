...
public
class CTNewLines implements CTPacket
{
    ...
    public
    CTNewLines(UUID world, short ticks, List<CTLine> lines)
    {
        this.world = world;
        this.ticks = ticks;
        this.lines = lines;
    ...
    public static
    CTNewLines fromBytes(DataInputStream dis) throws IOException
    {
        UUID  worldUUID = UUID.fromString(dis.readUTF());
        short tickTime  = dis.readShort();

        return new CTNewLines(worldUUID, tickTime, CTNewLines.getLines(dis, tickTime));
    }

    public static
    List<CTLine> getLines(DataInputStream dis, short ticks) throws IOException
    {
