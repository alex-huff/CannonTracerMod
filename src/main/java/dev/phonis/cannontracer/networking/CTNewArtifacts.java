...
public
class CTNewArtifacts implements CTPacket
{
    ...
    public
    CTNewArtifacts(UUID world, short ticks, List<CTArtifact> artifacts)
    {
        this.world     = world;
        this.ticks     = ticks;
    ...
    public static
    CTNewArtifacts fromBytes(DataInputStream dis) throws IOException
    {
        UUID  worldUUID = UUID.fromString(dis.readUTF());
        short tickTime  = dis.readShort();

        return new CTNewArtifacts(worldUUID, tickTime, CTNewArtifacts.getArtifacts(dis, tickTime));
    }

    private static
    List<CTArtifact> getArtifacts(DataInputStream dis, short ticks) throws IOException
    {
